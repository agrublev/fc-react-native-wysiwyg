...
function convertFcHtmlToEditor(inText) {
    inText = inText.trim();
    inText = inText.split("\n").map(e => e.trim()).join("");

    console.warn("-- Console BEFOORE", inText);
    // Bold / Strong tag
    ...
    inText = inText.replace(
        /\<h1\>(.[\s\S]*?)\<\/h1\>/g,
        `<h1><span style="font-weight: bold;">$1</span></h1>`
    );
    // Remove space between tags
    inText = inText.replace(
        /\<\/(.*?)\>(\s\S)\<(.*?)\>/g,
        `<$1><span style="font-weight: bold;">$2<\/$3>`
    );
    // Remove tabs
    inText = inText.replace(/\\t/g, ``);
    inText = inText.replace(/\t/g, ``);
    // Nbsp
    inText = inText.replace(/\&nbsp\;/g, ` `);
    // NewLines
    if (inText.slice(0, 4) !== "<div") {
        inText = "<div>" + inText + "</div>"
    }

    return inText;}

export function convertToObject(htmlString, styleList = null, fromFreedcamp = true) {
    if (fromFreedcamp) {
        htmlString = convertFcHtmlToEditor(htmlString);
    }
    //
    // console.warn("-- ConsoleBE", htmlString);
    // htmlString = htmlString.split("\n").join("");
    // console.warn("-- ConsoleAFT", htmlString);
    const availableStyles = styleList == null ? defaultStyles : styleList;
